...
void USBChecker::init_myusb_device(void) throw(int){
	...
	libusb_set_debug(ctx, LIBUSB_LOG_LEVEL_DEBUG); 	//todos los msjs seran mostrados en el archivo
													// registro de eventos
...
void /*USBChecker::*/close_myusb_device(int ignored){
	cout << "Sali con la senal " << ignored << endl;
	to_exit = 0;
}
...
void USBChecker::dataGenerator(unsigned char* data){
	...
	//para debug
//	out_timestamp();
//	cout << "Genere datos aleatorios:" << endl;
...
//		cout << "malisimo" << endl;
	...
	out_timestamp();
	cout << "Mande datos " << c << endl;

}

void USBChecker::send_cb(struct libusb_transfer* transfer){
	out_timestamp();
	cout << "Estoy en el callback de envio" << endl;
	...
	switch(transfer->status)
	{
		...
		case LIBUSB_TRANSFER_NO_DEVICE:
			...
			err_timestamp();
			cerr << "El dispositivo se desconecto. Se cierra programa." << endl;
			to_exit = 0;
			return;
		case LIBUSB_TRANSFER_OVERFLOW:
			err_timestamp();
			cerr << "Fallo por overflow. Revisar codigo de generacion de datos. Se cierra programa." << endl;
			to_exit = 0;
			break;
		default:
			break;
...
//    transfer_in->flags = LIBUSB_TRANSFER_FREE_BUFFER | LIBUSB_TRANSFER_FREE_TRANSFER;
	...
	cout << "Tengo " << pktSize << " como tamano de paquete" << endl;
}
...
int main() {
	...
	freopen("errlog","a", stderr);	// redirijo el stderr a un archivo de registro de eventos
									// la funcion freopen reinicia tbn cerr para que vaya al
									// mismo archivo
	...
	catch(int e)
	{
		...
		case NO_DOI:// 			901	//No device of interest
			cerr << "No se encontro dispoitivo. Conecte la placa Cypress" << endl;
			break;
	...
	while(to_exit)
	{
		...
		if(to_log)
		{
			...
			to_send = 1;
			cout << "le dije que haga mas envio de datos" << endl;
			to_log = 0;
			to_exit = 1;
		}
		tv.tv_sec = 0;
