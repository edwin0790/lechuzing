...
\begin{document}
	\begin{tikzpicture}[scale=1]
		\begin{scope}[transform shape,node distance=1,>=latex,]
			\node[moore,text width=100] (inicio) [label=above right:inicio]{FIFOADR="ZZ"\\SLOE='0'\\SLRD='0'};
			\node[ask,text width=100] (vacio1) [below=of inicio]{FLAG Vacío};
			\draw[->] (inicio.south) -| (vacio1);
			\node[moore,text width=100] (lecdir) [below=of vacio1] {FIFOADR=entrada\\SLOE='0'\\SLRD='0'};
			\draw[o->] (vacio1.east) -- ++ (1,0)  -- ++ (0,+5.5)-| (inicio.north);
			\draw[->] (vacio1.south) -| (lecdir);
			\node[ask,text width=100] (vacio2)[below=of lecdir]{FLAG Vacío};
			\draw[->](lecdir.south) -| (vacio2);
		\end{scope}
	\end{tikzpicture}
\end{document}